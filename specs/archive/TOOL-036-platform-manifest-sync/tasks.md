...
- **`compute_total_services` returns `len(stg) + len(prd) + 2`.** The `+2` was introduced in `ba1a2dd6` to keep the derived figure at 35 — the literal this change set removed. Removing it moves a number published on a public page (33, or 39 if the discarded `shared` table belongs in it); which is correct is a product decision, not a cleanup.
